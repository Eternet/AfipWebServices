...
internal class Program
{
    ...
    private static async Task Main(string[] args)
    {
        ...
        var prodCert = Path.Combine(current, configuration["AfipSettings:ProdCert"]);
        var testCert = Path.Combine(current, configuration["AfipSettings:TestCert"]);

        var envProd = new AfipEnvironment(
            long.Parse(configuration["AfipSettings:ProdCuit"]),
            true,
            prodCert,
            configuration["AfipSettings:Password"]
        );

        var envTest = new AfipEnvironment(
            long.Parse(configuration["AfipSettings:TestCuit"]),
            false,
            testCert,
            configuration["AfipSettings:Password"]
        );
